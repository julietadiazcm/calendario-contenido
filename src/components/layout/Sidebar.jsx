export function Sidebar({ accounts, activeAccountId, onSelectAccount, tab, onSelectTab, view, B, notifCount, open, onClose }) {
  const menuItems = [
    ["calendar", "📅 Calendario"],
  ...
  return (
    <>
      ...
      {open && (
        <div
          className="cm-sidebar-backdrop"
          onClick={onClose}
          style={{ display: "none" }} // shown via mobile CSS
        />
      )}

      <aside
        className={`cm-sidebar${open ? " cm-sidebar-open" : ""}`}
        style={{
          ...
          // mobile: overridden via CSS class (fixed overlay)
        }}
      >
        ...
        {accounts.map(acc => {
          ...
          return (
            <button
              key={acc.id}
              onClick={() => handleNav(onSelectAccount, acc.id)}
              style={{
        ...
        {menuItems.map(([id, label]) => (
          <button
            key={id}
            onClick={() => handleNav(onSelectTab, id)}
            style={{
              display: "block", width: "100%", padding: "10px 18px", border: "none",
              textAlign: "left", cursor: "pointer", fontSize: 14, transition: "all 0.15s",
